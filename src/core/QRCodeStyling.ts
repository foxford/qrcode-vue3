import type { Extension, Options, QRCode, RequiredOptions } from '@/types'
import * as qr from 'qrcode-generator'
import { downloadURI, getMode, mergeDeep, sanitizeOptions } from '@/tools'
...
export class QRCodeStyling {
  ...
  constructor(options?: Partial<Options>) {
    this._options = options ? sanitizeOptions(mergeDeep(QROptions, options) as RequiredOptions) : QROptions
    this.update()
  }
  ...
  async getImageUrl(extension: string): Promise<string> {
    ...
    if (getImageUrl === undefined) {
      if (!this._canvas)
        return ''
      return this._canvas.getCanvas().toDataURL(`image/${extension}`)
    }
    return ''
  }
